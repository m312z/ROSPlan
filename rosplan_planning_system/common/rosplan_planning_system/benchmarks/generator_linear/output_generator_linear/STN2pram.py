...
for key,value in sorted (dict_action.iteritems()):
	...
	if key != "0" and val1 < total_action:
		...
		if val1 < k:
			...
			stn_file.write("c: n" + str(val1) + ".start - .zero in [dur_" + str(val1) + ",dur_" + str(val1) + "];\n")
		...
		#action duration
		stn_file.write("c: n" + str(val1) + ".end - n" + str(val1) + ".start in [" + str(dict_action[key]) + "," + str( dict_action[key]) + "];\n")
