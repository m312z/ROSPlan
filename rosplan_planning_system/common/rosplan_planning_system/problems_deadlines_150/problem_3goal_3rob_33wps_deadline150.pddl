(define (problem task)
(:domain turtlebot)
(:objects
    wp0 wp1 wp2 - waypoint
    printer0 printer1 printer2 - printer
    robot0 robot1 robot2 - robot
)
(:init
    (robot_at robot0 wp2)
    (nocarrying_papers robot0)
    (undocked robot0)
    (localised robot0)

    (robot_at robot1 wp0)
    (nocarrying_papers robot1)
    (undocked robot1)
    (localised robot1)

    (robot_at robot2 wp1)
    (nocarrying_papers robot2)
    (undocked robot2)
    (localised robot2)

    (dock_at wp0)

    (delivery_destination wp0)  (at 900.0045 (not (delivery_destination wp0)))
    (delivery_destination wp1)  (at 900.0045 (not (delivery_destination wp1)))
    (delivery_destination wp2)  (at 900.0045 (not (delivery_destination wp2)))

    (= (distance wp0 wp1) 18)
    (= (distance wp1 wp0) 18)
    (= (distance wp0 wp2) 10)
    (= (distance wp2 wp0) 10)
    (= (distance wp0 printer0) 5)
    (= (distance printer0 wp0) 5)
    (= (distance wp0 printer1) 11)
    (= (distance printer1 wp0) 11)
    (= (distance wp0 printer2) 29)
    (= (distance printer2 wp0) 29)
    (= (distance wp1 wp2) 8)
    (= (distance wp2 wp1) 8)
    (= (distance wp1 printer0) 17)
    (= (distance printer0 wp1) 17)
    (= (distance wp1 printer1) 11)
    (= (distance printer1 wp1) 11)
    (= (distance wp1 printer2) 11)
    (= (distance printer2 wp1) 11)
    (= (distance wp2 printer0) 9)
    (= (distance printer0 wp2) 9)
    (= (distance wp2 printer1) 3)
    (= (distance printer1 wp2) 3)
    (= (distance wp2 printer2) 19)
    (= (distance printer2 wp2) 19)
    (= (distance printer0 printer1) 10)
    (= (distance printer1 printer0) 10)
    (= (distance printer0 printer2) 28)
    (= (distance printer2 printer0) 28)
    (= (distance printer1 printer2) 18)
    (= (distance printer2 printer1) 18)
)
(:goal (and
    (papers_delivered wp2)
    (papers_delivered wp0)
    (papers_delivered wp1)
))
)
